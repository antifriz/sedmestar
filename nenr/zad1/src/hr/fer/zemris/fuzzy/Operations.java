...
public class Operations {
    ...
    public static IBinaryFunction hamacherTNorm(double alpha) {
        return (domainAValue, domainBValue) -> (domainAValue * domainBValue) / (alpha + (1 - alpha) * (domainAValue + domainBValue - domainAValue * domainBValue));
    }

    public static IBinaryFunction hamacherSNorm(double alpha) {
        return (domainAValue, domainBValue) -> (domainAValue + domainBValue - (2 - alpha) * domainAValue * domainBValue) / (1 - (1 - alpha) * domainAValue * domainBValue);
    }
